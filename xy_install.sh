...
function user_select4() {
    ...
    for i in {1..5}; do
        if [[ $ok_115 =~ ^[Yy]$ ]]; then
            remote_size=$(curl -sL -D - -o /dev/null --max-time 10 "$docker_addr/d/ailg_jf/115/${down_path}/$emby_ailg" | grep "Content-Length" | cut -d' ' -f2 | tail -n 1 | tr -d '\r')$docker_addr/d/ailg_jf/115/ailg_img/${down_path}/$emby_ailg
        else
